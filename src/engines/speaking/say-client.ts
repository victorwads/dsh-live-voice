...
export class SayClientEngine {
  ...
  async capability() {
    try { return { ...await this.request('capabilities'), local: true, location: 'host' }; }
    catch (error) { return { supported: false, local: true, location: 'host', reason: `macOS say connection failed: ${error.message}` }; }
  }
  async speak(text, { voice, rate = 1, signal } = {}) {
    ...
    try {
      ...
      await this.request('speak', {
        clientId: this.clientId, operationId: operation.operationId,
        text, ...(voice === undefined ? {} : { voice }), rate: Math.round(rate * 175),
