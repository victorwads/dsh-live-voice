...
const aborted = (reason) => Object.assign(new Error('Speech audio conversion was cancelled.'), { name: 'AbortError', cause: reason });

/** Converts internal PCM WAV synthesis output to compact browser-playable AAC in M4A. */
export async function wavToM4aAac(input, {
  fs = nodeFs,
  run = execFile,
  tempRoot = tmpdir(),
  signal,
  ...
  try {
    ...
    const source = join(directory, 'speech.wav'), target = join(directory, 'speech.m4a');
