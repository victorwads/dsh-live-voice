...
export class HostAudioSpeakingEngine {
  ...
  async prepare(text, { voice, lang = this.lang, rate = 1, signal } = {}) {
    ...
    try {
      const response = await this.g.fetch(this.endpoint, {
        ...
        body: JSON.stringify({ text, lang, voice, rate: this.synthesisRate(rate) }), signal: controller.signal,
      });
      if (!response.ok) {
        let body;
      ...
      let disposed = false;
      return { audio: new this.g.Audio(url), url, dispose: () => { if (!disposed) { disposed = true; this.g.URL.revokeObjectURL(url); } } };
    } catch (error) {
  ...
  async playPrepared(prepared, options = {}) {
    ...
    return new Promise((resolve, reject) => {
      const cleanup = () => {
        audio.removeEventListener('ended', done); audio.removeEventListener('error', failed); signal?.removeEventListener('abort', aborted);
        if (this.current === operation) this.current = null;
        prepared.dispose();
      };
      const done = () => { cleanup(); resolve(); };
      const failed = () => { cleanup(); reject(new Error('The browser could not play host speech audio.')); };
      const aborted = () => { audio.pause(); cleanup(); reject(cancelled()); };
  ...
  async speak(text, options = {}) {
    ...
    try { return await this.playPrepared(prepared, options); }
  ...
  pause() { if (!this.current?.audio || this.current.audio.paused) return false; this.current.audio.pause(); return true; }
  async resume() { if (!this.current?.audio || !this.current.audio.paused) return false; await this.current.audio.play(); return true; }
}
