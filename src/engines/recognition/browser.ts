// @ts-nocheck
const abortError = () => Object.assign(new Error('Speech recognition was cancelled.'), { name: 'AbortError' });
const localReason = 'Local browser speech recognition is unavailable for this language. Install the local language pack, or explicitly turn off local-only processing in Settings if you permit the browser recognition service to process audio.';
const notify = (callback, value) => { try { callback?.(value); } catch { /* Consumer errors must not retain microphone ownership. */ } };
...
export class BrowserRecognitionEngine {
  ...
  async capability({ lang = this.lang, processLocally = this.processLocally } = {}) {
    ...
    } catch { return { supported: false, local:processLocally, reason: processLocally ? localReason : 'Browser speech recognition is unavailable in this browser.' }; }
  }
  ...
  async start({ lang = this.lang, signal, onResult = this.onResult, onActivity = this.onActivity, onError = this.onError } = {}) {
    ...
    const cancelled = new Promise((resolve, reject) => { session.rejectCancelled = reject; });
    session.cancel = () => { if (this.session === session) this.stop(); };
    signal?.addEventListener('abort', session.cancel, { once: true });
    const capability = await Promise.race([this.capability({ lang,processLocally:this.processLocally }), cancelled]);
    if (this.session !== session || signal?.aborted) { session.cancel(); throw abortError(); }
    if (!capability.supported) { this.stop(); throw new Error(capability.reason); }
    try {
      this._begin(session);
  ...
  _begin(session) {
    ...
    recognition.onspeechend = () => { if (valid()) this._activity(session, false); };
    recognition.onresult = event => {
      ...
      for (let i = 0; i < event.results.length; i++) {
        ...
        if (result.isFinal) {
          if (!finals.has(i)) { finals.add(i); final.push(text); }
    ...
    recognition.onend = () => {
      ...
      session.timer = (this.globals.setTimeout ?? globalThis.setTimeout)(() => {
        session.timer = null;
        if (this.session !== session) return;
        try { this._begin(session); } catch (error) { this._fail(session, error); }
  ...
  async stop() {
    ...
    try { session.recognition?.abort(); } catch { /* Browser already ended capture. */ }
    this._activity(session, false);
  }
}
