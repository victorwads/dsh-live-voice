...
export function apply(ctx) {
  ...
  function retire(entry) {
    ...
    entry.closed=true; entry.request++;
    entry.composers.clear();
    entry.unsubscribe?.(); entry.unsubscribe=null; entry.chatListeners.clear();
  ...
  function get(sessionId) {
    ...
    const entry={key,draft:'',pendingDraft:undefined,composers:new Map(),refs:0,buttons:0,request:0,closed:false};
    let settings={};
    try {settings=normalizeSettings(JSON.parse(localStorage.getItem('dsh-live-voice.settings') || '{}'));} catch {settings=normalizeSettings(null);}
    ...
    entry.controller=new VoiceCoordinator({recognition,engines:{browser:engineBrowser,say:engineSay},meter,
      composer:{getDraft:()=>entry.draft,submit:()=>{const owner=[...entry.composers.values()].at(-1);owner?.actions.submit?.();},setDraft:text=>{
        ...
        entry.draft=text; entry.pendingDraft=text; owner.actions.setDraft(text);
      }},settings});
    ...
    entry.subscribeChat=listener=>{
      if(entry.closed) return ()=>{};
      entry.chatListeners.add(listener); return ()=>entry.chatListeners.delete(listener);
    };
    ...
    const refresh=(baseline=false)=> {
      ...
      for(const message of assistantMessages(snapshot)) controller.observeMessage(message.id,message.text,{complete:message.complete,baseline:baseline || message.interrupted});
    };
    refresh(true);
    entry.unsubscribe=entry.chat.subscribe(()=>{refresh(); for(const listener of entry.chatListeners) listener();});
    const update=controller.updateSettings.bind(controller);
    controller.updateSettings=next=>{
      if(disposed || entry.closed) return;
      update(next); engineBrowser.lang=controller.getSnapshot().settings.lang;
      try{localStorage.setItem('dsh-live-voice.settings',JSON.stringify(controller.getSnapshot().settings));}catch{}
    ...
    for(const method of ['stopListening','cancelDictation','endConversation','stopSpeech']) {
      ...
      controller[method]=(...args)=>{entry.request++; return original(...args);};
    }
  ...
  function useComposer(entry,props) {
    ...
    React.useLayoutEffect(()=> {
      ...
      return ()=>{
        ...
        if(!entry.composers.size) {
          entry.request++;
          queueMicrotask(()=>{if(!entry.closed && !entry.composers.size) run(entry.controller,entry.controller.endConversation());});
        }
      };
    },[entry]);
    React.useLayoutEffect(()=> {
    ...
    React.useLayoutEffect(()=> {
      ...
      else if(published===entry.pendingDraft) {entry.draft=published;entry.pendingDraft=undefined;}
      entry.controller.composerChanged(entry.draft);
    });
  }
  function Buttons(props) {
    const entry=useEntry(props.sessionId,'buttons'); useComposer(entry,props);
  ...
  function Settings() {
    ...
    React.useEffect(()=>{
      ...
      const meter=new MicrophoneMeter();const recognition=recognitionFor(settings,meter);
      const c=new VoiceCoordinator({recognition,engines:{browser,say:new SayClientEngine({rpc:ctx.connection.rpc})},meter,composer:{getDraft:()=>'',setDraft:()=>{}},settings});
      ...
      const update=c.updateSettings.bind(c);let settingsRevision=0;
      c.updateSettings=async next=>{
        const revision=++settingsRevision;
        const previousEngine=c.getSnapshot().settings.recognitionEngine;
        update(next);browser.lang=c.getSnapshot().settings.lang;
        const settings=c.getSnapshot().settings;
        if(previousEngine!==settings.recognitionEngine)c.replaceRecognition(recognitionFor(settings,c.meter));
        ...
        for(const entry of controllers.values()) {
          ...
          if(entry.controller.getSnapshot().settings.recognitionEngine!==settings.recognitionEngine) entry.controller.replaceRecognition(recognitionFor(settings,entry.controller.meter));
      ...
      setController(c);refresh();
      return ()=>{document.removeEventListener('dsh-live-voice:capabilitieschanged',refresh);void c.dispose();};
    },[]);
  ...
  function Dock(props) {
    const entry=useEntry(props.sessionId,'dock'); useComposer(entry,props);
    return entry ? e(RecordingBar,{controller:entry.controller}) : null;
  }
  function ActionView({entry,messageId}) {
    const snapshot=React.useSyncExternalStore(entry.controller.subscribe,entry.controller.getSnapshot);
    ...
    return e(SpeakButton,{active,disabled:!message.text.trim() || (!active && unavailable),label:unavailable ? (capability?.reason || 'Checking speech output…') : undefined,onClick:()=>run(entry.controller,active ? entry.controller.stopSpeech() : entry.controller.speak(message.text,message.id))});
  }
  function Action(props) {
    const entry=useEntry(props.sessionId,'action');
    return entry ? e(ActionView,{entry,messageId:props.messageId}) : null;
  }
  ctx.effect(()=>{const style=document.createElement('style');style.dataset.plugin='dsh-live-voice';style.textContent=styles;document.head.appendChild(style);return()=>style.remove();});
  ...
  ]) ctx.slots.inject(name,()=>ctx.slots.register({name,id,order,label:'DSH Live Voice'},component));
  ctx.effect(()=> {
    const stop=()=>{ownership.cancel();for(const entry of controllers.values()) run(entry.controller,entry.controller.endConversation());};
    const onKey=event=>{
      ...
      event.preventDefault(); const c=candidates[0].controller;
      run(c,c.getSnapshot().listening || c.getSnapshot().starting ? c.stopListening() : c.startDictation());
    };
    const refreshCapabilities=()=>{for(const entry of controllers.values()) run(entry.controller,entry.controller.refreshCapabilities());document.dispatchEvent(new Event('dsh-live-voice:capabilitieschanged'));};
    const visibilityChanged=()=>{if(document.visibilityState==='visible') refreshCapabilities();};
    ...
    return()=>{
      ...
      document.removeEventListener('keydown',onKey); window.removeEventListener('pagehide',stop);
