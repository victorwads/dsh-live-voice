...
function pendingQuestionSpeech(interaction) {
  if (!interaction || interaction.kind !== "question" || !Array.isArray(interaction.questions)) return "";
  return interaction.questions.map((item) => typeof item?.question === "string" ? item.question.trim() : "").filter(Boolean).join(" ");
}
...
function apply(ctx) {
  ...
  function get(sessionId) {
    ...
    const refreshPendingQuestion = (baseline = false) => {
      ...
      if (text && controller.getSnapshot().conversation) run(controller, controller.speak(text, key2));
    };
