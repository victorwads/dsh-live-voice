...
var voiceDetectionPresets = Object.freeze({
  short: Object.freeze({ silenceMs: 900, label: "Short", description: "Send quickly after a short pause." }),
  natural: Object.freeze({ silenceMs: 1500, label: "Natural", description: "Allow normal pauses between phrases." }),
  long: Object.freeze({ silenceMs: 2200, label: "Long", description: "Wait through longer thinking pauses." })
});
var usesPluginVoiceDetection = (engine) => ["whisper-http"].includes(engine);
var defaultSettings = Object.freeze({ engine: "browser", recognitionEngine: "browser", recognitionProcessLocally: true, recognitionAutoInstall: true, voiceDetectionPreset: "natural", announceAssistantMessages: true, interruptSpeechOnUserMessage: false, sendingMode: "manual", autoSendDelaySeconds: 4, assistantSpeechDelaySeconds: 3, mode: "speaker", lang: "pt-BR", recognitionLang: "pt-BR", voice: "", rate: 1 });
function normalizeSettings(value) {
  const source = value && typeof value === "object" && !Array.isArray(value) ? value : {};
  return {
    engine: ["browser", "say"].includes(source.engine) ? source.engine : defaultSettings.engine,
    recognitionEngine: ["browser", "whisper-http"].includes(source.recognitionEngine) ? source.recognitionEngine : defaultSettings.recognitionEngine,
...
var VoiceCoordinator = class {
  constructor({ recognition, engines, meter, composer, settings = {} }) {
    ...
    this.snapshot = { conversation: false, listening: false, recognizing: false, speaking: false, paused: false, starting: false, error: null, activeMessageId: null, autoSendAt: null, capabilities: {}, settings: normalizeSettings(settings) };
  ...
  replaceRecognition(recognition) {
    ...
    Object.assign(recognition, { lang: this.snapshot.settings.recognitionLang, processLocally: this.snapshot.settings.recognitionProcessLocally, autoInstallLocalPack: this.snapshot.settings.recognitionAutoInstall, voiceDetectionPreset: this.snapshot.settings.voiceDetectionPreset });
    this.patch({ capabilities: { ...this.snapshot.capabilities, recognition: void 0 } });
  }
  explainRecognition() {
    throw new Error((this.snapshot.capabilities.capture?.reason || this.snapshot.capabilities.recognition?.reason || "Local speech recognition is unavailable.") + " Open Settings \u2192 Live Voice to check language and engine availability.");
  }
  updateSettings(next) {
    if (this.disposed) return;
    const settings = normalizeSettings({ ...this.snapshot.settings, ...next });
    Object.assign(this.recognition, { lang: settings.recognitionLang, processLocally: settings.recognitionProcessLocally, autoInstallLocalPack: settings.recognitionAutoInstall, voiceDetectionPreset: settings.voiceDetectionPreset });
  ...
  async refreshCapabilities() {
    ...
    Object.assign(this.recognition, { lang, processLocally: this.snapshot.settings.recognitionProcessLocally, autoInstallLocalPack: this.snapshot.settings.recognitionAutoInstall });
    const probes = Object.entries(this.engines).map(([id2, engine]) => [id2, () => engine.capability?.() ?? engine.getCapabilities()]);
    probes.push(["recognition", () => this.recognition.capability({ lang })]);
    probes.push(["capture", () => this.meter.capability()]);
    await Promise.all(probes.map(async ([id2, probe]) => {
      try {
  ...
  _startInput(conversation, prerequisite = Promise.resolve()) {
    ...
    return this._input(async () => {
      ...
      try {
        ...
        const capability = await cancellable(this.recognition.capability({ lang }), controller.signal);
        if (!valid()) return;
        if (!capability.supported) throw new Error(capability.reason || "Local browser recognition is unavailable.");
        let started;
        try {
          started = await this.meter.start({ signal: controller.signal });
        } catch (error) {
          if (valid()) this.patch({ capabilities: { ...this.snapshot.capabilities, capture: { supported: false, permission: error?.name === "NotAllowedError" ? "denied" : "error", reason: error?.name === "NotAllowedError" ? "Microphone permission was denied. Allow it in browser settings, then refresh availability." : `Microphone capture failed: ${message(error)}` } } });
          throw error;
        }
        if (!valid()) {
          await this._releaseInput();
        ...
        await this.recognition.start({
          ...
          onActivity: (active) => {
            ...
            if (active) {
              ...
              this._cancelAssistantSpeechTimer();
            } else this.assistantSpeechNotBefore = Date.now() + this.snapshot.settings.assistantSpeechDelaySeconds * 1e3;
        ...
        if (valid()) {
          this.queue = [];
          this.patch({ starting: false, listening: false, conversation: false, recognizing: false, error: message(error) });
        }
      }
    });
  }
  onResult({ final = "", interim = "" }) {
    ...
    if (!interim && this.snapshot.recognizing) this.assistantSpeechNotBefore = Date.now() + this.snapshot.settings.assistantSpeechDelaySeconds * 1e3;
  ...
  scheduleAutoSend(draft) {
    ...
    this.autoSendTimer = setTimeout(() => {
      ...
      if (!this.disposed && this.snapshot.settings.sendingMode === "automatic" && expected === this.composer.getDraft()) this.composer.submit();
    }, delay);
  }
  cancelAutoSend() {
  ...
  async stopSpeech(resumeListening = true) {
    ...
    const stopped = this.speechBarrier.then(async () => {
      const results = await Promise.allSettled(Object.values(this.engines).map((engine) => Promise.resolve().then(() => engine.stop())));
    ...
    if (epoch === this.speechEpoch && !this.disposed && resumeListening && this.snapshot.conversation && !this.snapshot.listening && !this.snapshot.starting) await this._startInput(true);
  }
  async pauseSpeech() {
    ...
    try {
      ...
      if (epoch === this.speechEpoch && control === this.controlEpoch && this.snapshot.speaking && result !== false) this.patch({ paused: true });
    } catch (error) {
      if (epoch === this.speechEpoch) this.patch({ error: message(error) });
    }
  }
  async resumeSpeech() {
    ...
    try {
      ...
      if (epoch === this.speechEpoch && control === this.controlEpoch && result !== false) this.patch({ paused: false });
    } catch (error) {
      if (epoch === this.speechEpoch) this.patch({ error: message(error) });
    }
  }
  async speak(text, messageId = null) {
    ...
    const epoch = this.speechEpoch;
    await stopped;
    if (!this.disposed && epoch === this.speechEpoch && !this.speechStopError) return this.play(text, messageId);
  }
  async play(text, messageId) {
    ...
    try {
      ...
      if (epoch !== this.speechEpoch || this.disposed) return;
      await engine.speak(text, { voice: this.snapshot.settings.voice || void 0, rate: this.snapshot.settings.rate });
    } catch (error) {
      ...
      if (epoch === this.speechEpoch && !this.disposed) {
        ...
        else if (this.snapshot.conversation && !this.snapshot.listening && !this.snapshot.starting) await this._startInput(true);
      }
    }
  }
  ...
  _drain() {
    if (this.disposed || !this.snapshot.conversation || !this.snapshot.settings.announceAssistantMessages || this.snapshot.speaking || this.snapshot.recognizing || this.snapshot.starting || !this.queue.length) return;
    const wait = this.assistantSpeechNotBefore - Date.now();
    if (wait > 0) {
      if (Number.isFinite(wait) && this.assistantSpeechTimer === null) this.assistantSpeechTimer = setTimeout(() => {
        this.assistantSpeechTimer = null;
        this._drain();
      }, wait);
      return;
  ...
  observeMessage(id2, text, { complete = false, baseline = false } = {}) {
    ...
    const boundary = complete ? remaining.length : Math.max(remaining.lastIndexOf(". "), remaining.lastIndexOf("? "), remaining.lastIndexOf("! "), remaining.lastIndexOf("\n")) + 1;
...
var VoiceOwnership = class {
  ...
  run(owner, owners, action) {
    ...
    const acquired = this.tail.then(async () => {
      ...
      await Promise.all(owners.filter((other) => other !== owner).map((other) => other.endConversation()));
      if (this.closed || epoch !== this.epoch || owner.disposed) return;
...
var BrowserSpeakingEngine = class {
  ...
  voices() {
    try {
      return Array.from(this.globals.speechSynthesis?.getVoices?.() ?? []).filter((v) => v.localService === true);
    } catch {
      return [];
    }
  }
  ...
  async speak(text, { voice, rate = 1, signal } = {}) {
    ...
    const chosen = voice == null ? voices.find((v) => v.lang?.toLowerCase() === this.lang.toLowerCase()) ?? voices[0] : voices.find((v) => typeof voice === "string" ? v.voiceURI === voice || v.name === voice : v === voice);
...
var SayClientEngine = class {
  ...
  async request(endpoint, payload = {}, signal) {
    ...
    if (!result?.ok) {
      ...
      throw Object.assign(new Error(result?.error?.message || "Host speech is unavailable."), { code: result?.error?.code });
    }
    return result.value;
  }
  async capability() {
    try {
      ...
      return { supported: false, local: true, location: "host", reason: `macOS say connection failed: ${error.message}` };
    }
  }
  async speak(text, { voice, rate = 1, signal } = {}) {
    if (typeof text !== "string") throw new TypeError("Speech text must be a string.");
  ...
  async stop() {
    ...
    if (!operation) return;
    this.current = null;
    const stopping = this.request("stop", { clientId: this.clientId, operationId: operation.operationId });
  ...
  async control(endpoint) {
    ...
    const value = await this.request(endpoint, { clientId: this.clientId, operationId: operation.operationId });
...
var BrowserRecognitionEngine = class {
  constructor({ globals = globalThis, lang = "pt-BR", processLocally = true, autoInstallLocalPack = true, onResult, onActivity, onError, maxRestarts = 3, restartDelayMs = 100 } = {}) {
    if (!Number.isInteger(maxRestarts) || maxRestarts < 0 || maxRestarts > 100) throw new RangeError("maxRestarts must be an integer between 0 and 100.");
    if (!Number.isFinite(restartDelayMs) || restartDelayMs < 0) throw new RangeError("restartDelayMs must be nonnegative.");
    Object.assign(this, { globals, lang, processLocally, autoInstallLocalPack, onResult, onActivity, onError, maxRestarts, restartDelayMs });
    this.session = null;
  }
  get active() {
  ...
  async capability({ lang = this.lang, processLocally = this.processLocally } = {}) {
    try {
      ...
      if (!processLocally) return { supported: true, local: false, reason: "Browser recognition service may process audio remotely." };
      if (typeof Recognition.available !== "function" || !("processLocally" in probe)) throw new Error();
      ...
      if (availability === "downloadable" && this.autoInstallLocalPack) {
        ...
        if (installed === true) availability = await Recognition.available({ langs: [lang], processLocally: true });
        else return { supported: false, local: true, availability, reason: `The browser could not install the local ${lang} language pack. Disable local processing to use the browser recognition service, or try again later.` };
      }
      if (availability !== "available") return { supported: false, local: true, availability, reason: `Local recognition for ${lang}: ${availability}. ${availability === "downloadable" ? "The language pack is not installed; enable automatic installation or browser-service recognition." : availability === "downloading" ? "The browser is still installing the language pack." : "The browser cannot currently provide on-device recognition for this language."}` };
      return { supported: true, local: true };
    } catch {
      return { supported: false, local: processLocally, reason: processLocally ? localReason : "Browser speech recognition is unavailable in this browser." };
    }
  }
  async installLocalPack({ lang = this.lang } = {}) {
    const Recognition = this.Recognition;
    if (typeof Recognition?.install !== "function") throw new Error("This browser cannot install local speech-recognition language packs.");
  ...
  async start({ lang = this.lang, signal, onResult = this.onResult, onActivity = this.onActivity, onError = this.onError } = {}) {
    this.stop();
    if (signal?.aborted) throw abortError2();
    const session = { lang, signal, onResult, onActivity, onError, restarts: 0, timer: null, recognition: null, speaking: false };
    ...
    const capability = await Promise.race([this.capability({ lang, processLocally: this.processLocally }), cancelled2]);
  ...
  _detach(recognition) {
    ...
    for (const name of ["onresult", "onerror", "onend", "onspeechstart", "onspeechend"]) recognition[name] = null;
  }
  _fail(session, error) {
  ...
  _begin(session) {
    ...
    recognition.onerror = (event) => {
      if (!valid()) return;
      const error = Object.assign(new Error("Local browser recognition failed: " + (event.error || "unknown error")), { code: event.error });
    ...
    recognition.onend = () => {
      ...
      if (session.restarts >= this.maxRestarts) {
        this._fail(session, Object.assign(new Error("Local browser recognition stopped repeatedly. Restart listening manually or choose another local engine."), { code: "restart-limit" }));
        return;
      }
      session.restarts++;
  ...
  async stop() {
    ...
    if (session.timer !== null) (this.globals.clearTimeout ?? globalThis.clearTimeout)(session.timer);
...
var WhisperHttpRecognitionEngine = class {
  ...
  async capability() {
    try {
      const response = await this.g.fetch(ROUTE + "/capabilities", { credentials: "same-origin" }), json = await response.json();
      return json?.ok ? json.value : { supported: false, local: true, streaming: false, reason: json?.error?.message || "Whisper HTTP host is unavailable." };
    } catch (error) {
      return { supported: false, local: true, streaming: false, reason: "Whisper HTTP host connection failed: " + error.message };
    }
  }
  async start({ lang = this.lang, signal, onResult, onActivity, onError } = {}) {
    ...
    if (!context || !source || typeof context.createScriptProcessor !== "function") throw new Error("This browser cannot capture PCM audio for Whisper HTTP.");
    ...
    const session = { operation: id(), processor, gain, chunks: [], samples: 0, voiced: false, silence: 0, inflight: /* @__PURE__ */ new Set(), onResult, onActivity, onError, lang, signal };
    this.session = session;
    ...
    const submit = async () => {
      ...
      try {
        const response = await this.g.fetch(ROUTE + "/transcribe", { method: "POST", credentials: "same-origin", headers: { "content-type": "audio/wav", "x-dlv-client-id": session.operation, "x-dlv-operation-id": id(), "x-dlv-language": lang }, body: encodeMonoPcm16Wav(samples, context.sampleRate), signal: request.signal });
        const json = await response.json();
        if (!response.ok || !json?.ok) throw new Error(json?.error?.message || "Whisper HTTP transcription failed.");
    ...
    processor.onaudioprocess = (event) => {
      ...
      if (session.voiced && session.silence > context.sampleRate * (this.segmentation.silenceMs / 1e3) || session.samples > context.sampleRate * 20) void submit();
    };
...
async function whisperSettingsRequest(path, { method = "GET", config, signal } = {}, fetchImpl = globalThis.fetch) {
  const response = await fetchImpl(BASE + path, { method, credentials: "same-origin", signal, headers: config ? { "content-type": "application/json" } : void 0, body: config ? JSON.stringify(config) : void 0 });
...
function createWhisperSettings(React2) {
  ...
  return function WhisperSettings({ controller }) {
    const [draft, setDraft] = React2.useState({ url: "http://127.0.0.1:8080/inference", healthUrl: "/health", timeoutMs: 3e4 });
      ...
      try {
        if (action === "load") {
          ...
          if (!abort.signal.aborted) {
            ...
            setMessage("Connection successful. Health endpoint responded; transcription was not tested. Unsaved edits have not been applied.");
          }
        }
      } catch (reason) {
    ...
    function field(label, key, type = "text") {
      return h("label", null, label, h("input", { type, value: draft[key], disabled: busy || !loaded, autoComplete: "off", ...type === "number" ? { min: 100, max: 3e5, step: 1 } : {}, onChange: (event) => {
...
function createComponents(React2) {
  ...
  function Button({ label, icon, className = "dlv-pill-button", ...props }) {
    return h("button", { ...props, type: "button", className: "dlv-icon-button " + className, title: label, "aria-label": label }, h(Icon, { name: icon }));
  }
  ...
  function ErrorText({ error, onDismiss }) {
    return error ? h(
      ...
      String(error),
      onDismiss ? h("button", { type: "button", "aria-label": "Dismiss voice error", onClick: onDismiss }, "Dismiss") : null
    ) : null;
  }
  function MicrophoneButtons({ controller }) {
    ...
    return h(
      React2.Fragment,
      null,
      h(Button, { className: "dlv-mic", icon: "mic", label: pending ? "Checking microphone availability" : unavailable ? reason || "Speech recognition unavailable" : "Voice typing", disabled: pending, onClick: () => unavailable ? invoke("explainRecognition") : invoke("startDictation") }),
      h(Button, { className: "dlv-mic", icon: "conversation", label: pending ? "Checking microphone availability" : unavailable ? reason || "Speech recognition unavailable" : "Start voice conversation", disabled: pending, onClick: () => unavailable ? invoke("explainRecognition") : invoke("startConversation") }),
  ...
  function RecordingBar({ controller }) {
    ...
    if (!state.conversation && !capture && !state.speaking && !state.paused && !state.error && !error) return null;
    ...
    return h(
      ...
      h(
        ...
        capture && !state.conversation ? h(Button, { label: "Cancel dictation", icon: "close", onClick: () => invoke("cancelDictation") }) : null,
        state.conversation ? h(Button, { label: "End voice conversation", icon: "close", onClick: () => invoke("endConversation") }) : null,
        h(Waveform, { controller, enabled: Boolean(state.listening) }),
        h("span", { className: "dlv-status", role: "status", "aria-live": "polite" }, status),
        remaining ? h(Button, { label: "Cancel automatic send", icon: "close", onClick: () => invoke("cancelAutoSend") }) : null,
        state.conversation && !capture ? h(Button, { label: "Take microphone", icon: "mic", onClick: () => invoke("startConversation") }) : null,
        capture ? h(Button, { label: "Stop listening", icon: "stop", onClick: () => invoke("stopListening") }) : null,
        state.speaking && !state.paused && state.capabilities[state.settings.engine]?.pause ? h(Button, { label: "Pause speech", icon: "pause", onClick: () => invoke("pauseSpeech") }) : null,
        state.paused && state.capabilities[state.settings.engine]?.resume ? h(Button, { label: "Resume speech", icon: "play", onClick: () => invoke("resumeSpeech") }) : null,
        state.speaking || state.paused ? h(Button, { label: "Stop all speech", icon: "stop", onClick: () => invoke("stopSpeech") }) : null
      ),
      h(ErrorText, { error: error || state.error, onDismiss: () => {
        clearError();
  ...
  function SpeakButton({ active = false, disabled = false, label, onClick }) {
    return h(Button, { className: "dlv-speaker", label: label || (active ? "Stop speaking" : "Speak message"), icon: active ? "stop" : "speaker", "aria-pressed": Boolean(active), disabled, onClick });
  }
  function SettingsPanel({ controller, onClose }) {
    ...
    const field = (label, key, options) => h(
      ...
      h(
        "select",
        { value: settings[key] || options[0].value, onChange: (event) => invoke("updateSettings", key === "engine" ? { engine: event.target.value, voice: "" } : { [key]: event.target.value }) },
        options.map((option) => h("option", { key: option.value, value: option.value, disabled: option.disabled }, option.label))
      )
    );
    const card = (title, children, open = false) => h("details", { className: "dlv-settings-card", open }, h("summary", null, title), h("div", { className: "dlv-settings-card-body" }, ...children));
    const subcard = (title, children, open = false) => h("details", { className: "dlv-settings-subcard", open }, h("summary", null, title), h("div", { className: "dlv-settings-subcard-body" }, ...children));
    return h(
      "section",
      ...
      h("details", { className: "dlv-settings-card" }, h("summary", null, "Speech output"), h(
        "div",
        { className: "dlv-settings-card-body" },
        field("Speech engine", "engine", [
          { value: "say", label: "macOS say \u2014 host audio", disabled: capabilities.say?.supported === false },
          { value: "browser", label: "Browser speech \u2014 device audio", disabled: capabilities.browser?.supported === false }
        ]),
        settings.engine === "browser" ? field("Local browser voice", "voice", [{ value: "", label: "Automatic local voice" }, ...(capabilities.browser?.voices || []).map((voice) => ({ value: voice.voiceURI || voice.name, label: `${voice.name} \u2014 ${voice.lang || "unknown language"}` }))]) : h("label", null, "macOS say voice (empty uses system default)", h("input", { type: "text", value: settings.voice || "", onChange: (event) => invoke("updateSettings", { voice: event.target.value }), autoComplete: "off" })),
        h("label", null, "Speech rate", h("input", { type: "number", min: 0.1, max: 3, step: 0.1, value: settings.rate ?? 1, onChange: (event) => {
          const rate = Number(event.target.value);
          if (Number.isFinite(rate) && rate >= 0.1 && rate <= 3) invoke("updateSettings", { rate });
        } }), h("small", null, "Relative speed: 1 is normal.")),
        h("p", null, "macOS say plays on the DSH host. Browser speech plays on this device.")
      )),
      h("details", { className: "dlv-settings-card" }, h("summary", null, "Speech recognition"), h(
        "div",
        { className: "dlv-settings-card-body" },
        h("label", null, "Recognition engine", h("select", { value: settings.recognitionEngine, onChange: (event) => invoke("updateSettings", event.target.value === "browser" && settings.recognitionLang === "auto" ? { recognitionEngine: "browser", recognitionLang: "pt-BR" } : { recognitionEngine: event.target.value }) }, h("option", { value: "browser" }, "Browser SpeechRecognition"), h("option", { value: "whisper-http" }, "Whisper HTTP \u2014 DSH host"))),
        field("Recognition language", "recognitionLang", [...settings.recognitionEngine === "whisper-http" ? [{ value: "auto", label: "Automatic \u2014 detect language" }] : [], { value: "pt-BR", label: "Portugu\xEAs (Brasil)" }, { value: "en-US", label: "English (United States)" }]),
        settings.recognitionEngine === "browser" ? h(
          React2.Fragment,
          null,
          h("label", { className: "dlv-check" }, h("input", { type: "checkbox", checked: settings.recognitionProcessLocally !== false, onChange: (event) => invoke("updateSettings", { recognitionProcessLocally: event.target.checked }) }), " Process recognition locally on this device"),
          settings.recognitionProcessLocally !== false ? h("label", { className: "dlv-check" }, h("input", { type: "checkbox", checked: settings.recognitionAutoInstall !== false, onChange: (event) => invoke("updateSettings", { recognitionAutoInstall: event.target.checked }) }), " Automatically install this browser language pack when needed") : h("p", { role: "status" }, "Browser-service recognition is enabled. The browser may send microphone audio to its recognition service.")
        ) : h("p", { role: "status" }, "Audio is segmented into complete WAV utterances, sent through the authenticated DSH host, and processed by loopback whisper.cpp HTTP. This server does not support true realtime streaming."),
        ...
        usesPluginVoiceDetection(settings.recognitionEngine) ? h(
          ...
          h(
            ...
            h("p", null, "Controls how long a pause must last before captured speech is sent for recognition."),
            h(
              "div",
              { className: "dlv-preset-group", role: "radiogroup", "aria-label": "Pause before sending" },
              Object.entries(voiceDetectionPresets).map(([value, preset]) => h(
                "label",
                { key: value, className: "dlv-preset" },
                h("input", { type: "radio", name: "dlv-vad-preset", value, checked: (settings.voiceDetectionPreset || "natural") === value, onChange: () => invoke("updateSettings", { voiceDetectionPreset: value }) }),
                h("span", null, h("strong", null, preset.label), h("small", null, preset.description))
              ))
            ),
            h("p", { className: "dlv-vad-summary" }, "Pause before sending: " + (voiceDetectionPresets[settings.voiceDetectionPreset]?.silenceMs || voiceDetectionPresets.natural.silenceMs) + " ms")
          )
        ) : null
      )),
      card("Conversation", [
        h("label", { key: "announce", className: "dlv-check" }, h("input", { type: "checkbox", checked: settings.announceAssistantMessages !== false, onChange: (event) => invoke("updateSettings", { announceAssistantMessages: event.target.checked }) }), " Automatically speak new assistant messages"),
        h("p", { key: "policy" }, "During a voice conversation, assistant phrases are announced automatically. Playback waits while you are speaking."),
        h("label", { key: "interrupt-message", className: "dlv-check" }, h("input", { type: "checkbox", checked: settings.interruptSpeechOnUserMessage === true, onChange: (event) => invoke("updateSettings", { interruptSpeechOnUserMessage: event.target.checked }) }), " Stop assistant speech when I send a message"),
        h("p", { key: "interrupt-message-description", className: "dlv-setting-description" }, settings.interruptSpeechOnUserMessage ? "Sending or steering a new user message stops current or paused assistant speech." : "Sending another message does not stop the assistant audio you are already hearing."),
        field("Listening mode", "mode", [{ value: "speaker", label: "Speakers \u2014 gated listening" }, { value: "headphones", label: "Headphones \u2014 open microphone" }]),
        h("p", { key: "mode-description", className: "dlv-setting-description" }, settings.mode === "headphones" ? "Open microphone keeps listening while responses play. When your speech is detected, playback pauses and resumes only when you choose." : "Gated listening releases the microphone while responses play, preventing speaker audio from being recognized. Use Take microphone to interrupt."),
        h("label", { key: "speech-delay" }, "Assistant response delay", h("select", { value: String(settings.assistantSpeechDelaySeconds || 3), onChange: (event) => invoke("updateSettings", { assistantSpeechDelaySeconds: Number(event.target.value) }) }, [1, 2, 3, 4, 5, 6, 8, 10].map((seconds) => h("option", { key: seconds, value: String(seconds) }, seconds + " seconds"))), h("small", null, "After you stop speaking, automatic assistant playback waits for this much continuous silence. Speaking again restarts the wait.")),
        field("Sending mode", "sendingMode", [{ value: "manual", label: "Manual \u2014 review and send" }, { value: "automatic", label: "Automatic \u2014 send after silence" }]),
        settings.sendingMode === "automatic" ? h("label", { key: "delay" }, "Send after silence", h("select", { value: String(settings.autoSendDelaySeconds || 4), onChange: (event) => invoke("updateSettings", { autoSendDelaySeconds: Number(event.target.value) }) }, [2, 3, 4, 5, 6, 8, 10].map((seconds) => h("option", { key: seconds, value: String(seconds) }, seconds + " seconds"))), h("small", null, "Countdown starts after a final recognized phrase. New speech or edits cancel it.")) : h("p", { key: "manual", className: "dlv-setting-description" }, "Recognized text stays in the composer until you use the normal DSH Send control.")
      ], false),
      capabilities.capture?.supported === false ? h("p", { role: "status" }, `Microphone: ${capabilities.capture.reason}`) : capabilities.capture?.permission === "prompt" ? h("p", { role: "status" }, "Microphone permission will be requested only when you start dictation or a voice conversation.") : null,
      capabilities.recognition?.supported === false ? h("p", { role: "status" }, capabilities.recognition.reason) : null,
      ...["say", "browser"].filter((id2) => capabilities[id2]?.supported === false).map((id2) => h("p", { key: id2, role: "status" }, `${id2 === "say" ? "macOS say" : "Browser speech"}: ${capabilities[id2].reason}`)),
      h(
        "div",
        { className: "dlv-settings-actions" },
        h("button", { type: "button", disabled: capabilities[settings.engine]?.supported !== true || state.speaking, onClick: () => invoke("speak", "DSH Live Voice. The selected speech output is working.") }, state.speaking ? "Testing speech\u2026" : "Test selected speech output"),
        state.speaking || state.paused ? h("button", { type: "button", onClick: () => invoke("stopSpeech") }, "Stop speech test") : null,
        h("button", { type: "button", onClick: () => invoke("refreshCapabilities") }, "Refresh available engines")
      ),
      h(ErrorText, { error: error || state.error, onDismiss: () => {
        clearError();
        controller.clearError();
...
function apply(ctx) {
  ...
  const recognitionFor = (settings, meter) => settings.recognitionEngine === "whisper-http" ? new WhisperHttpRecognitionEngine({ meter, voiceDetectionPreset: settings.voiceDetectionPreset }) : new BrowserRecognitionEngine({ processLocally: settings.recognitionProcessLocally, autoInstallLocalPack: settings.recognitionAutoInstall });
  const run = (controller, promise) => Promise.resolve(promise).catch((error) => {
    if (!disposed && !controller.disposed) controller.patch({ error: error?.message || String(error) });
  });
  ...
  function get(sessionId) {
    ...
    const entry = { key, draft: "", pendingDraft: void 0, composers: /* @__PURE__ */ new Map(), refs: 0, buttons: 0, request: 0, closed: false };
    let settings = {};
    try {
      settings = normalizeSettings(JSON.parse(localStorage.getItem("dsh-live-voice.settings") || "{}"));
    } catch {
    ...
    entry.controller = new VoiceCoordinator({
      recognition,
      engines: { browser: engineBrowser, say: engineSay },
      meter,
      composer: { getDraft: () => entry.draft, submit: () => {
    ...
    const refresh = (baseline = false) => {
      ...
      if (!baseline && userSeq > entry.userSeq && controller.getSnapshot().settings.interruptSpeechOnUserMessage && (controller.getSnapshot().speaking || controller.getSnapshot().paused)) run(controller, controller.stopSpeech());
      entry.userSeq = Math.max(entry.userSeq ?? -1, userSeq);
      for (const message2 of assistantMessages(snapshot)) controller.observeMessage(message2.id, message2.text, { complete: message2.complete, baseline: baseline || message2.interrupted });
    };
    refresh(true);
    ...
    controller.updateSettings = (next) => {
      ...
      try {
        localStorage.setItem("dsh-live-voice.settings", JSON.stringify(controller.getSnapshot().settings));
      } catch {
      }
    ...
    for (const method of ["startDictation", "startConversation", "speak"]) {
      ...
      controller[method] = (...args) => {
        ...
        return ownership.run(controller, [...controllers.values()].map((other) => other.controller).concat([...retiring]), () => {
    ...
    import_react.default.useLayoutEffect(() => {
      ...
      return () => {
        ...
        if (!entry.composers.size) {
          entry.request++;
          queueMicrotask(() => {
            if (!entry.closed && !entry.composers.size) run(entry.controller, entry.controller.endConversation());
          });
        }
      };
    }, [entry]);
  ...
  function Settings() {
    ...
    import_react.default.useEffect(() => {
      ...
      const meter = new MicrophoneMeter();
      const recognition = recognitionFor(settings, meter);
      const c = new VoiceCoordinator({ recognition, engines: { browser, say: new SayClientEngine({ rpc: ctx.connection.rpc }) }, meter, composer: { getDraft: () => "", setDraft: () => {
      } }, settings });
      const speak = c.speak.bind(c);
      c.speak = (...args) => ownership.run(c, [...controllers.values()].map((entry) => entry.controller).concat([...retiring]), () => speak(...args));
      ...
      c.updateSettings = async (next) => {
        ...
        if (previousEngine !== settings2.recognitionEngine) c.replaceRecognition(recognitionFor(settings2, c.meter));
        ...
        await Promise.allSettled([c.endConversation(), ...active.map((entry) => entry.controller.endConversation())]);
        if (revision !== settingsRevision || disposed || c.disposed) return;
        for (const entry of controllers.values()) {
          if (entry.closed) continue;
          if (entry.controller.getSnapshot().settings.recognitionEngine !== settings2.recognitionEngine) entry.controller.replaceRecognition(recognitionFor(settings2, entry.controller.meter));
  ...
  ctx.slots.inject("settings.section", () => ctx.slots.register({ name: "settings.section", id: "dsh-live-voice", order: 65, label: "Live Voice" }, Settings));
  function Dock(props) {
  ...
  function ActionView({ entry, messageId }) {
    const snapshot = import_react.default.useSyncExternalStore(entry.controller.subscribe, entry.controller.getSnapshot);
    ...
    return e(SpeakButton, { active, disabled: !message2.text.trim() || !active && unavailable, label: unavailable ? capability?.reason || "Checking speech output\u2026" : void 0, onClick: () => run(entry.controller, active ? entry.controller.stopSpeech() : entry.controller.speak(message2.text, message2.id)) });
  }
  ...
  ]) ctx.slots.inject(name, () => ctx.slots.register({ name, id: id2, order, label: "DSH Live Voice" }, component));
  ctx.effect(() => {
    const stop = () => {
      ownership.cancel();
      for (const entry of controllers.values()) run(entry.controller, entry.controller.endConversation());
    };
    const onKey = (event) => {
      if (disposed || event.defaultPrevented || !event.ctrlKey || !event.shiftKey || event.code !== "Space" || event.repeat) return;
      const candidates = [...controllers.values()].filter((entry) => entry.buttons > 0 && entry.composers.size > 0);
      if (candidates.length !== 1) return;
      event.preventDefault();
      const c = candidates[0].controller;
      run(c, c.getSnapshot().listening || c.getSnapshot().starting ? c.stopListening() : c.startDictation());
    };
    const refreshCapabilities = () => {
      for (const entry of controllers.values()) run(entry.controller, entry.controller.refreshCapabilities());
