...
// src/engines/speaking/m4a-aac.ts
...
var aborted2 = (reason) => Object.assign(new Error("Speech audio conversion was cancelled."), { name: "AbortError", cause: reason });
async function wavToM4aAac(input, {
  fs = nodeFs2,
  run = execFile,
  tempRoot = tmpdir2(),
  signal,
  ...
  if (signal?.aborted) throw aborted2(signal.reason);
  let directory, result, error;
  try {
    directory = await fs.mkdtemp(join2(tempRoot, "dsh-live-voice-aac-"));
    await fs.chmod(directory, 448);
    const source = join2(directory, "speech.wav"), target = join2(directory, "speech.m4a");
    await fs.writeFile(source, input, { mode: 384, flag: "wx" });
    if (signal?.aborted) throw aborted2(signal.reason);
    await run("/usr/bin/afconvert", ["-f", "m4af", "-d", "aac", "-b", String(bitrate), source, target], {
      shell: false,
      signal
    });
    if (signal?.aborted) throw aborted2(signal.reason);
    ...
    error = signal?.aborted && caught?.name !== "AbortError" ? aborted2(signal.reason) : caught;
  } finally {
    if (directory) {
      try {
...
function createWhisperConfigStore(path = join3(homedir(), ".dsh", "dsh-live-voice-whisper.json")) {
  return {
    async load() {
      try {
...
function createQwenConfigStore(path = join4(homedir2(), ".dsh", "dsh-live-voice-qwen.json")) {
  return {
    async load() {
      try {
