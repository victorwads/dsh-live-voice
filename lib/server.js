...
function aborted(reason) {
  return Object.assign(new Error("Speech cancelled", reason === void 0 ? void 0 : { cause: reason }), {
    name: "AbortError",
    code: "ABORT_ERR"
  });
}
var SayEngine = class {
  ...
  #control(from, to, signal) {
    if (this.#platform !== "darwin" || this.#state !== from || !this.#active?.child || this.#active.cancelled) return false;
    try {
      if (!this.#active.child.kill(signal)) throw failure("Unable to signal speech process", "SAY_SIGNAL_FAILED");
      this.#state = to;
      return true;
    } catch (error) {
  ...
  async #run(request, text, voice, rate) {
    ...
    try {
      check();
      if (this.#unclosed) throw failure("Previous speech process has not closed", "SAY_PROCESS_UNCLOSED");
      this.#active = request;
  ...
  #waitForClose(request, child) {
    return new Promise((resolve, reject) => {
      ...
      const send = (signal) => {
        try {
          if (!child.kill(signal)) processError ??= failure("Unable to signal speech process", "SAY_SIGNAL_FAILED");
        } catch (error) {
          processError ??= error;
        }
      };
      ...
      child.once("close", (code, signal) => {
        ...
        else if (code !== 0) reject(failure("Speech process exited unsuccessfully", "SAY_EXIT_FAILED", { code, signal }));
        else resolve();
      });
      request.cancelChild = () => {
        if (closed) return;
        ...
        killTimer = setTimeout(() => {
          ...
          closeTimer = setTimeout(() => {
            if (closed) return;
            this.#unclosed = child;
            reject(failure("Speech process did not close after cancellation", "SAY_STOP_TIMEOUT", processError));
          }, this.#closeAfterMs);
        }, this.#killAfterMs);
      };
...
function validateWhisperConfig(value) {
  if (!value || typeof value.url !== "string" || typeof value.healthUrl !== "string") throw new Error("Endpoint URL and health URL/path are required.");
  ...
  if (!Number.isInteger(value.timeoutMs) || value.timeoutMs < 100 || value.timeoutMs > 3e5) throw new Error("Request timeout must be an integer between 100 and 300000 ms.");
...
function resolveWhisperUrl(value = process.env.DSH_LIVE_VOICE_WHISPER_URL || "http://127.0.0.1:8080/inference") {
  ...
  if (url.protocol !== "http:" || !LOOPBACK.has(url.hostname) || url.username || url.password || url.hash) throw new Error("Whisper HTTP URL must be an unauthenticated loopback http URL.");
  return url;
}
function validateMonoPcm16Wav(input, { maxBytes = 2e6 } = {}) {
  const bytes = input instanceof Uint8Array ? input : new Uint8Array(input);
  if (bytes.byteLength < 44 || bytes.byteLength > maxBytes) throw new Error("Invalid or oversized WAV recording.");
  ...
  if (ascii(0, 4) !== "RIFF" || ascii(8, 4) !== "WAVE" || view.getUint32(4, true) + 8 !== bytes.byteLength) throw new Error("Invalid WAV container.");
  ...
  if (!fmt || fmt.size < 16 || !data || at !== bytes.byteLength) throw new Error("Incomplete WAV recording.");
  const f = fmt.start;
  if (view.getUint16(f, true) !== 1 || view.getUint16(f + 2, true) !== 1 || view.getUint32(f + 4, true) !== 16e3 || view.getUint32(f + 8, true) !== 32e3 || view.getUint16(f + 12, true) !== 2 || view.getUint16(f + 14, true) !== 16 || data.size < 2 || data.size % 2) throw new Error("WAV must be mono 16 kHz PCM16.");
  return bytes;
}
var WhisperHttpHost = class {
  constructor({ url, healthUrl = "/health", timeoutMs = 3e4, fetchImpl = globalThis.fetch, maxBytes = 2e6, store } = {}) {
  ...
  async request(url, options, config, consume = (response) => response) {
    ...
    const signal = AbortSignal.any([timeout, this.active.signal, ...options.signal ? [options.signal] : []]);
  ...
  async capability(signal, configuration) {
    try {
      ...
      const response = await this.request(new URL(config.healthUrl, config.url), { signal }, config);
      return response.ok ? { supported: true, local: true, streaming: false, maxBytes: this.maxBytes } : { supported: false, local: true, streaming: false, reason: "Whisper HTTP health check failed (" + response.status + ")." };
    } catch (error) {
      return { supported: false, local: true, streaming: false, reason: "Whisper HTTP server is unreachable: " + (error?.message || error) };
    }
  }
  async transcribe(input, { lang = "auto", signal } = {}) {
    ...
    return this.request(new URL(config.url), { method: "POST", body: form, signal }, config, async (response) => {
      if (!response.ok) throw new Error("Whisper HTTP transcription failed (" + response.status + ").");
...
function createSayHost({ engine = new SayEngine() } = {}) {
  ...
  async function handle(endpoint, payload, signal) {
    ...
    try {
      ...
      if (!["speak", "stop", "pause", "resume"].includes(endpoint)) return fail("not-found", "Unknown speech endpoint.");
      ...
      if (typeof payload.text !== "string" || !payload.text.trim() || payload.text.length > 1e5 || payload.text.includes("\0")) return fail("invalid-request", "Speech text must contain 1\u2013100000 characters without NUL.");
      if (payload.voice !== void 0 && (typeof payload.voice !== "string" || !payload.voice.trim() || payload.voice.length > 200 || payload.voice.includes("\0"))) return fail("invalid-request", "Invalid voice.");
      if (!Number.isFinite(payload.rate) || payload.rate < 18 || payload.rate > 1750) return fail("invalid-request", "Invalid speech rate.");
      if (active && active.clientId !== payload.clientId) return fail("busy", "Another voice client owns the host speaker.");
      if (owns) return fail("duplicate-operation", "The speech operation is already active.");
      active?.abort.abort();
      const operation = { clientId: payload.clientId, operationId: payload.operationId, abort: new AbortController(), done: null };
      active = operation;
      const cancel = () => operation.abort.abort();
      signal?.addEventListener("abort", cancel, { once: true });
      if (signal?.aborted) cancel();
      operation.done = Promise.resolve().then(() => engine.speak(payload.text, {
        voice: payload.voice,
        rate: payload.rate,
        signal: operation.abort.signal
      })).then(() => ok({ completed: true }), (error) => error?.name === "AbortError" ? fail("cancelled", "Speech was cancelled.") : fail("speech-failed", "Local speech failed."));
      try {
...
function apply(ctx, { whisperStore = createWhisperConfigStore(), whisperFetch = globalThis.fetch } = {}) {
  ...
  for (const endpoint of ["config", "test"]) {
    const dispose = ctx.connection.fetch.register({ path: SAY_CHANNEL + "/whisper/" + endpoint, methods: endpoint === "config" ? ["GET", "PUT"] : ["POST"], requestBody: "buffered", fetch: async (request) => {
      try {
        ...
        return Response.json(fail("config-failed", error.message || "Whisper settings could not be saved."), { status: 500 });
      }
    } });
  ...
  const whisperCapability = ctx.connection.fetch.register({ path: SAY_CHANNEL + "/whisper/capabilities", methods: ["GET"], requestBody: "buffered", fetch: async (request) => Response.json(ok(await whisper.capability(request.signal))) });
  const whisperTranscribe = ctx.connection.fetch.register({ path: SAY_CHANNEL + "/whisper/transcribe", methods: ["POST"], requestBody: "buffered", fetch: async (request) => {
    try {
      const clientId = request.headers.get("x-dlv-client-id"), operationId = request.headers.get("x-dlv-operation-id");
      if (!identity(clientId) || !identity(operationId)) return Response.json(fail("invalid-request", "Valid client and operation IDs are required."), { status: 400 });
      if (request.headers.get("content-type")?.split(";")[0] !== "audio/wav") return Response.json(fail("invalid-audio", "A mono 16 kHz PCM WAV recording is required."), { status: 400 });
      const length = Number(request.headers.get("content-length") || 0);
      if (length > whisper.maxBytes) return Response.json(fail("audio-too-large", "The recording exceeds the configured Whisper limit."), { status: 413 });
      const value = await whisper.transcribe(await request.arrayBuffer(), { lang: request.headers.get("x-dlv-language") || "auto", signal: request.signal });
      return Response.json(ok(value));
    } catch (error) {
      if (error?.name === "AbortError") return Response.json(fail("cancelled", "Whisper transcription was cancelled."), { status: 499 });
      return Response.json(fail("transcription-failed", error?.message || "Whisper transcription failed."), { status: 502 });
    }
  } });
  ctx.effect(() => async () => {
  ...
  for (const endpoint of ["capabilities", "speak", "stop", "pause", "resume"]) {
    const dispose = ctx.connection.fetch.register({
      ...
      fetch: async (request) => {
        ...
        if (body?.type !== "client-request" || typeof body.rpcId !== "string" || body.rpcId.length > 128 || body.method !== `dsh-live-voice/${endpoint}`) return new Response("Invalid RPC envelope", { status: 400 });
