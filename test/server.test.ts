...
test('shared API route adapter validates envelopes and disposes every registration', async()=>{
 ...
 apply({connection:{fetch:{register(route){routes.set(route.path,route);return ()=>routes.delete(route.path);}}},effect(fn){cleanup.push(fn());}});
 assert.equal(routes.size,9);
 assert.equal(routes.get(SAY_CHANNEL+'/whisper/transcribe').requestBody,'buffered');
 const invalidAudio=await routes.get(SAY_CHANNEL+'/whisper/transcribe').fetch(new Request('http://localhost'+SAY_CHANNEL+'/whisper/transcribe',{method:'POST',headers:{'content-type':'audio/wav','x-dlv-client-id':'a','x-dlv-operation-id':'b'},body:new Uint8Array(44)}));
 assert.equal(invalidAudio.status,502);
 const route=routes.get(SAY_CHANNEL+'/stop');
 const request=body=>new Request('http://localhost'+route.path,{method:'POST',body:JSON.stringify(body)});
 assert.equal((await route.fetch(request({type:'wrong'}))).status,400);
 const response=await route.fetch(request({type:'client-request',rpcId:'test',method:'dsh-live-voice/stop',payload:{clientId:'a',operationId:'b'}}));
 assert.deepEqual(await response.json(),{type:'server-response',rpcId:'test',result:{ok:true,value:{applied:false}}});
...
test('SayClient sends valid SDK targets and envelopes to actual registered routes', async()=>{
 ...
 apply({connection:{fetch:{register(route){routes.set(route.path,route);return ()=>routes.delete(route.path);}}},effect(fn){cleanup.push(fn());}});
 const seen=[];
 const client=new SayClientEngine({rpc:{async call(channel,endpoint,payload,signal){
   assert.match(channel,/^\/[A-Za-z0-9._~-]+$/);
   const path=channel+'/'+endpoint;seen.push(path);
   const route=routes.get(path);assert.ok(route);
   const rpcId=crypto.randomUUID();
   const response=await route.fetch(new Request('http://localhost'+path,{method:'POST',body:JSON.stringify({type:'client-request',rpcId,method:endpoint,payload}),signal}));
   assert.equal(response.status,200);const body=await response.json();assert.equal(body.rpcId,rpcId);return body.result;
 }}});
 try {
   assert.deepEqual(await client.request('stop',{clientId:'test',operationId:'inactive'}),{applied:false});
  ...
  const engine = {
    ...
    speak(text, options) {
      return new Promise((resolve, reject) => {
        ...
        jobs.push(job);
        const abort = () => { job.aborted = true; reject(Object.assign(new Error(), { name: 'AbortError' })); };
        options.signal.addEventListener('abort', abort, { once: true });
        if (options.signal.aborted) abort();
      });
    },
    pause: () => true, resume: () => true, stop: async () => {},
  };
  const host = createSayHost({ engine });
  return { host, jobs, engine, rpc: { call(channel, endpoint, payload, signal) {
    assert.match(channel, /^\/[A-Za-z0-9._~-]+$/);
...
const payload = (operationId, clientId = 'client') => ({ clientId, operationId, text: 'Private text', rate: 175 });
...
test('speak stays pending until process completion; disconnect cancels', async () => {
  ...
  let finished = false;
  const speech = host.handle('speak', payload('first'), abort.signal).then(value => { finished = true; return value; });
  await tick();
...
test('client stop aborts operation and host disposal closes admission', async () => {
  ...
  assert.equal(jobs[0].aborted, true);
  let stopped = false;
  engine.stop = async () => { stopped = true; };
  await host.dispose();
...
test('failed engines return sanitized errors without transcript details', async () => {
  const host = createSayHost({ engine: { speak: async () => { throw new Error('secret text'); } } });
