...
function fixture({ pendingResume = false, pendingClose = false } = {}) {
  ...
  class AudioContext {
    constructor() { this.state = 'suspended'; this.resuming = deferred(); this.closing = deferred(); this.closed = 0; contexts.push(this); }
    createAnalyser() { return { fftSize: 256, getFloatTimeDomainData(samples) { samples.fill(0.1); } }; }
    createMediaStreamSource() { return { connect() {}, disconnect() {} }; }
    resume() { return pendingResume ? this.resuming.promise : Promise.resolve(); }
    close() { this.closed++; return pendingClose ? this.closing.promise : Promise.resolve(); }
  }
  const meter = new MicrophoneMeter({ AudioContext, navigator: { mediaDevices: { getUserMedia() { const request = deferred(); requests.push(request); return request.promise; } } } });
  return { meter, requests, contexts };
}

test('capture capability distinguishes security, APIs and denied permission',async()=>{
 assert.equal((await new MicrophoneMeter({location:{hostname:'remote.test'},isSecureContext:false,navigator:{mediaDevices:{getUserMedia(){}}},AudioContext:class{}}).capability()).supported,false);
 assert.match((await new MicrophoneMeter({location:{hostname:'localhost'},navigator:{},AudioContext:class{}}).capability()).reason,/does not expose microphone/);
 const denied=await new MicrophoneMeter({location:{hostname:'localhost'},navigator:{mediaDevices:{getUserMedia(){}},permissions:{query:async()=>({state:'denied'})}},AudioContext:class{}}).capability();
 assert.deepEqual(denied,{supported:false,permission:'denied',reason:'Microphone permission is denied. Allow it in browser settings, then refresh availability.'});
 const prompt=await new MicrophoneMeter({location:{hostname:'localhost'},navigator:{mediaDevices:{getUserMedia(){}}},AudioContext:class{}}).capability();
...
test('replacement starts without waiting on old permission and late streams cannot replace it', async () => {
  ...
  const fresh = stream(); requests[1].resolve(fresh);
...
test('aborted pending resume and close never block or close a newer context', async () => {
  for (const rejectResume of [true, false]) {
    ...
    const oldStream = stream(); requests[0].resolve(oldStream);
    await Promise.resolve();
    controller.abort();
    assert.equal(await first, false);
...
test('current resume failure rejects and releases only its own resources', async () => {
  ...
  const audio = stream(); requests[0].resolve(audio);
  await Promise.resolve();
...
test('abort after successful start releases metering and repeated stop is harmless', async () => {
  ...
  controller.abort();
  await meter.stop(); await meter.release();
  assert.equal(audio.track.stopped, 1);
  assert.equal(contexts[0].closed, 1);
  assert.equal(meter.level(), 0);
  assert.equal(getEventListeners(controller.signal, 'abort').length, 0);
