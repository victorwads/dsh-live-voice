...
test('secure UTF-8 file argv; only close plus cleanup resolves speech', async () => {
  ...
  const speech = engine.speak(text, { voice: 'Samantha', rate: 180 }).then(() => {
    settled = true;
  });
  await turn();
  ...
  assert.deepEqual(
    calls.filter(([name]) => name === 'chmod').map((call) => call[2]),
    [0o700, 0o600],
  );
  ...
  assert.equal(engine.lastError, null);
});
...
test('spawn throw, process error, and nonzero close clean temporary files', async () => {
  ...
  for (const emittedError of [true, false]) {
    ...
    const rejection = assert.rejects(
      speech,
      emittedError ? (error) => error === problem : { code: 'SAY_EXIT_FAILED' },
    );
    await turn();
    if (emittedError) {
      children[0].emit('error', problem);
      await turn();
      assert.equal(
        calls.some(([name]) => name === 'rm'),
        false,
      );
    }
...
test('pre-abort does not allocate or spawn and abort during preparation cleans', async () => {
  ...
  await assert.rejects(before.engine.speak('hello', { signal: controller.signal }), {
    name: 'AbortError',
  });
  ...
  await turn();
  running.abort();
  allocation.resolve('/fake-temp/private');
  await rejection;
  assert.equal(during.children.length, 0);
  assert.deepEqual(during.calls.at(-1), [
    'rm',
    '/fake-temp/private',
...
test('abort waits for close and cleanup; stop is asynchronous and idempotent', async () => {
  ...
  assert.equal(engine.state, 'stopping');
...
test('replacement serializes through cleanup and only latest queued request spawns', async () => {
  ...
  const { engine, children } = fixture({
    fs: { rm: () => (++removed === 1 ? cleanup.promise : Promise.resolve()) },
  });
  const first = assert.rejects(engine.speak('first'), { name: 'AbortError' });
  await turn();
...
test('pause and resume signal actual process; stopping paused speech wakes it first', async () => {
  ...
  const speech = assert.rejects(engine.speak('hello'), { name: 'AbortError' });
  await turn();
  ...
  children[0].emit('close', null, 'SIGTERM');
  await Promise.all([speech, stopped]);
});
...
test('unclosed child times out with error, cleans, and blocks overlapping speech', async () => {
  ...
  const speech = assert.rejects(engine.speak('hello'), { code: 'SAY_STOP_TIMEOUT' });
  await turn();
  await assert.rejects(engine.stop(), { code: 'SAY_STOP_TIMEOUT' });
  await speech;
  assert.equal(engine.state, 'error');
  assert.equal(calls.at(-1)[0], 'rm');
  ...
  const next = engine.speak('safe now');
  await turn();
  children[1].emit('close', 0, null);
  await next;
});
...
test('file preparation failure cleans; cleanup failure is exposed and never resolves', async () => {
  ...
  assert.equal(brokenWrite.children.length, 0);
  assert.equal(brokenWrite.calls.at(-1)[0], 'rm');
  const { engine, children } = fixture({
    fs: {
      rm: async () => {
  ...
  const speech = assert.rejects(engine.speak('hello'), { code: 'SAY_CLEANUP_FAILED' });
  await turn();
  children[0].emit('close', 0, null);
  await speech;
...
test('abort during cleanup rejects instead of publishing stale success', async () => {
  ...
  const controller = new AbortController();
  const { engine, children } = fixture({ fs: { rm: () => cleanup.promise } });
  const speech = assert.rejects(engine.speak('hello', { signal: controller.signal }), {
    name: 'AbortError',
  });
  await turn();
  children[0].emit('close', 0, null);
