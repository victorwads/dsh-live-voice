...
function synthesis(voices = [remote, local]) {
  ...
  const globals = {
    ...
    speechSynthesis: {
      cancelled: 0, paused: false,
      getVoices: () => voices,
      speak: utterance => spoken.push(utterance),
      cancel() { this.cancelled++; spoken.at(-1)?.onend?.(); },
...
function recognition({ availability = 'available', localProperty = true, startError, installResult = false } = {}) {
  const instances = [], checks = [], installs = [], timers = new Map();
  let nextTimer = 0;
  class Recognition {
    static async available(options) { checks.push(options); return typeof availability === 'function' ? availability() : availability; }
    static async install(options) { installs.push(options); return installResult; }
    constructor() { if (localProperty) this.processLocally = false; instances.push(this); }
    start() {
    ...
    emit(name, value = {}) { this['on' + name]?.(value); }
  }
  const globals = {
  ...
  return { globals, instances, checks, installs, timers, current: () => instances.at(-1), tick() { const entries = [...timers]; timers.clear(); for (const [, fn] of entries) fn(); } };
}
const result = (transcript, isFinal = false) => Object.assign([{ transcript }], { isFinal });
const detached = r => { for (const name of ['onresult', 'onerror', 'onend', 'onspeechstart', 'onspeechend']) assert.equal(r[name], null); };

test('synthesis reports and honors optional pause support truthfully',()=>{
 const {globals}=synthesis();delete globals.speechSynthesis.pause;delete globals.speechSynthesis.resume;
 const engine=new BrowserSpeakingEngine({globals});
 assert.deepEqual({pause:engine.capability().pause,resume:engine.capability().resume},{pause:false,resume:false});
 assert.equal(engine.pause(),false);assert.equal(engine.resume(),false);
});
...
test('synthesis exposes only local voices and completes on end, not enqueue', async () => {
  ...
  let complete = false;
  const pending = engine.speak('Olá', { rate: 1.2, signal: controller.signal }).then(() => { complete = true; });
  await Promise.resolve();
  assert.equal(complete, false);
  assert.equal(spoken[0].voice, local);
  assert.equal(spoken[0].lang, 'pt-BR');
  assert.equal(spoken[0].rate, 1.2);
  engine.pause(); assert.equal(globals.speechSynthesis.paused, true);
  engine.resume(); assert.equal(globals.speechSynthesis.paused, false);
  spoken[0].onend();
  await pending;
...
test('synthesis replacement, signal abort and stop reject once and ignore stale events', async () => {
  ...
  await engine.stop();
  await assert.rejects(engine.speak('never', { signal: controller.signal }), { name: 'AbortError' });
...
test('synthesis native errors and synchronous exceptions release handlers', async () => {
  ...
  assert.equal(getEventListeners(controller.signal, 'abort').length, 0);
  globals.speechSynthesis.speak = () => { throw new Error('native throw'); };
...
test('local-only recognition requires exact availability when automatic install is disabled', async () => {
  for (const options of [{ localProperty: false }, { availability: 'downloadable' }, { availability: 'downloading' }, { availability: 'unavailable' }, { availability: true }, { availability: () => { throw new Error('denied'); } }]) {
    const fake = recognition(options);
    const engine = new BrowserRecognitionEngine({ globals: fake.globals, autoInstallLocalPack:false });
    ...
    assert.equal(fake.instances.some(r => r.started), false);
...
test('recognition locality is user-controlled and local packs may install automatically',async()=>{
 const remote=recognition({localProperty:false});const remoteEngine=new BrowserRecognitionEngine({globals:remote.globals,processLocally:false});
 assert.deepEqual(await remoteEngine.capability(),{supported:true,local:false,reason:'Browser recognition service may process audio remotely.'});
 await remoteEngine.start();assert.equal(remote.current().processLocally,undefined);await remoteEngine.stop();
 let availability='downloadable';const local=recognition({availability:()=>availability,installResult:true});local.globals.SpeechRecognition.install=async options=>{local.installs.push(options);availability='available';return true;};
 const localEngine=new BrowserRecognitionEngine({globals:local.globals,processLocally:true,autoInstallLocalPack:true});
 assert.equal((await localEngine.capability()).supported,true);assert.deepEqual(local.installs,[{langs:['pt-BR'],processLocally:true}]);
});

test('recognition configures local continuous interim recognition, deduplicates finals, clears activity', async () => {
  const fake = recognition(), results = [], activity = [];
  const engine = new BrowserRecognitionEngine({ globals: fake.globals, onResult: r => results.push(r), onActivity: a => activity.push(a) });
  const controller = new AbortController();
  ...
  assert.equal(r.continuous, true); assert.equal(r.interimResults, true); assert.equal(r.lang, 'pt-BR');
  r.emit('speechstart'); r.emit('speechstart');
  ...
  r.emit('result', { resultIndex: 1, results: [result('Olá', true), result('mundo', true)] });
  assert.deepEqual(results, [{ interim: 'Olá', final: '' }, { interim: 'mundo', final: 'Olá' }, { interim: '', final: 'mundo' }, { interim: '', final: '' }]);
  const stale = r.onresult;
  await engine.stop();
  stale({ results: [result('ignored', true)] });
  assert.equal(results.length, 4);
  assert.deepEqual(activity, [true, false]);
  assert.equal(r.aborted, true); detached(r);
  assert.equal(getEventListeners(controller.signal, 'abort').length, 0);
});

test('recognition stop promptly cancels pending checks and later checks cannot resurrect capture', async () => {
  let release;
  const fake = recognition({ availability: () => new Promise(resolve => { release = resolve; }) });
  ...
  await rejected;
  assert.equal(getEventListeners(controller.signal, 'abort').length, 0);
  release('available'); await Promise.resolve(); await Promise.resolve();
  assert.equal(fake.instances.some(r => r.started), false);
  assert.equal(engine.active, false);
});
...
test('recognition overlapping starts accept only newest language and abort active sessions', async () => {
  const releases = [];
  const fake = recognition({ availability: () => new Promise(resolve => releases.push(resolve)) });
  const engine = new BrowserRecognitionEngine({ globals: fake.globals });
  const first = engine.start();
  const rejected = assert.rejects(first, { name: 'AbortError' });
  const controller = new AbortController();
  const second = engine.start({ lang: 'en-US', signal: controller.signal });
  releases[1]('available'); await second;
  releases[0]('available'); await rejected;
  assert.equal(fake.current().lang, 'en-US');
  assert.equal(fake.instances.filter(r => r.started).length, 1);
  controller.abort();
  assert.equal(engine.active, false); detached(fake.current());
...
test('recognition consecutive no-progress restarts are bounded', async () => {
  const fake = recognition(), errors = [], results = [], delays = [];
  const schedule = fake.globals.setTimeout;
  fake.globals.setTimeout = (fn, delay) => { delays.push(delay); return schedule(fn); };
  const engine = new BrowserRecognitionEngine({ globals: fake.globals, maxRestarts: 2, onError: e => errors.push(e), onResult: r => results.push(r) });
  await engine.start();
  for (let i = 0; i < 3; i++) {
    const r = fake.current();
    r.emit('result', { results: [result('')] });
    r.emit('end'); detached(r); fake.tick();
  }
  assert.equal(fake.instances.filter(r => r.started).length, 3);
  assert.equal(errors[0].code, 'restart-limit');
  assert.deepEqual(delays, [100, 200]);
  assert.equal(results.length, 3);
  assert.equal(engine.active, false); assert.equal(fake.timers.size, 0);
});
...
test('healthy recognition continues beyond restart limit and resets final indexes', async () => {
  for (const progress of ['result', 'speechstart']) {
    const fake = recognition(), results = [], errors = [];
    const engine = new BrowserRecognitionEngine({ globals: fake.globals, onResult: r => results.push(r), onError: e => errors.push(e) });
    await engine.start();
    for (let i = 0; i < 10; i++) {
      const r = fake.current();
    ...
    if (progress === 'result') assert.deepEqual(results.map(r => r.final), Array(10).fill('repeat'));
    await engine.stop();
  }
});
...
test('recognition capability safely rejects broken constructors and unwritable local flags', async () => {
  const constructors = [
    class { constructor() { throw new Error('constructor failed'); } static available() { return 'available'; } },
    class { constructor() { Object.defineProperty(this, 'processLocally', { value: false }); } start() {} abort() {} static available() { return 'available'; } },
  ...
  const globals = { get SpeechRecognition() { throw new Error('access denied'); } };
  assert.equal((await new BrowserRecognitionEngine({ globals }).capability()).supported, false);
  const fake = recognition();
  assert.equal((await new BrowserRecognitionEngine({ globals: { SpeechRecognition: {}, webkitSpeechRecognition: fake.globals.SpeechRecognition } }).capability()).supported, true);
});
...
test('recognition fatal errors stop without fallback; synchronous start errors clean up', async () => {
  const fake = recognition(), errors = [];
  const controller = new AbortController();
  const engine = new BrowserRecognitionEngine({ globals: fake.globals, onError: e => errors.push(e) });
  await engine.start({ signal: controller.signal });
  const r = fake.current();
  const end = r.onend;
  r.emit('error', { error: 'not-allowed' }); end(); fake.tick();
  assert.equal(errors[0].code, 'not-allowed');
  assert.equal(engine.active, false); detached(r);
  assert.equal(getEventListeners(controller.signal, 'abort').length, 0);
...
test('recognition no-speech may restart and consumer exceptions do not leak resources', async () => {
  const fake = recognition();
  const engine = new BrowserRecognitionEngine({ globals: fake.globals, onActivity() { throw new Error('consumer'); }, onError() { throw new Error('consumer'); }, onResult() { throw new Error('consumer'); } });
  await engine.start();
  const r = fake.current();
  r.emit('speechstart'); r.emit('result', { results: [result('text')] });
  r.emit('error', { error: 'no-speech' });
  assert.equal(engine.active, true);
  r.emit('end'); fake.tick();
  assert.equal(fake.instances.filter(r => r.started).length, 2);
  await engine.stop(); detached(fake.current());
});
